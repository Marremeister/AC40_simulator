...
class Maneuver:
    ...
    def plot_meters_lost(self):
        ...
        gybe_plotter = Plotter(x_values=gybe_times,
                               y_values=gybe_losses,
                               title="Meters Lost per Maneuver Over Time",
                               xlabel="Time of Maneuver Initiation (seconds)",
                               ylabel="Meters Lost",
                               plot_style="scatter")
        gybe_plotter.plot()
    ...
    def plot_best_maneuver(self, maneuver_type, y_columns=["Boat.VMG_kts", "Boat.Speed_kts", "Boat.TWA", "Boat.Heel"]):
        """
        ...
        maneuvers = sorted(maneuvers, key=lambda x: x[2])

        # Extract the start and end indices for the best maneuver (lowest VMG loss)
        best_maneuver = maneuvers[0]
        start, end, _, _ = best_maneuver

        # Extract data for the best maneuver, slicing every 10th value for both x and y-values
        maneuver_df = self.df.iloc[start:end:10]

        # Plot each specified column against time
        x_values = maneuver_df["Time"]
        for column in y_columns:
            if column in maneuver_df.columns:
                y_values = maneuver_df[column]
                plotter = Plotter(x_values, y_values, title=f"{maneuver_type.capitalize()} - {column}", xlabel="Time",
                                  ylabel=column)
                plotter.plot()
...
class Plotter:
    def __init__(self, x_values, y_values, title="", xlabel="", ylabel="", plot_style="line", ax=None):
