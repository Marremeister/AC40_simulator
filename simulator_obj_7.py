...
class Maneuver:
    ...
    def plot_best_maneuver(self, maneuver_type, y_columns=["Boat.VMG_kts", "Boat.Speed_kts", "Boat.TWA", "Boat.Heel", "Boat.FoilPort.Cant", "Boat.FoilStbd.Cant"]):
        ...
        if best_maneuver:
            ...
            # Adjust rows for the case when there are less than 4 columns
            if n <= 3:
                rows, cols = 1, n
            ...
            for idx, column in enumerate(y_columns):
                ...
                if column in maneuver_df.columns:
                    ...
                    single_column_plotter = Plotter(x_values=x_values,
                                                    title=f"{maneuver_type.capitalize()} Maneuver - {column}",
                                                    xlabel="Time",
                                                    ylabel="Value",
            ...
            # Show the individual subplots
            plt.tight_layout()
            plt.show()

            # Plot all columns together
            multi_column_plotter = Plotter(x_values=x_values,
                                           title=f"{maneuver_type.capitalize()} Maneuver - All Metrics",
                                           xlabel="Time",
                                           ylabel="Value")
            for column in y_columns:
                if column in maneuver_df.columns:
                    y_values = maneuver_df[column]
                    multi_column_plotter.add_line(y_values, label=column)

            multi_column_plotter.plot()
